...
async def on_number(message: Message, state: FSMContext) -> None:
    ...
    await message.answer(texts.CONFIRM_STEPS.format(steps=steps), parse_mode="Markdown",
...
async def on_confirm(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    if review:
        ...
        await notify_admins(cb.bot, f"⚠️ Проверка: {p['full_name']} прислал {steps} шагов (≥30k).")
